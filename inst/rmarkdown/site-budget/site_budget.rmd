...
if (!is.null(params$data_dir)) {
    register_supplementary_observations(params$data_dir)
}
```
...
read_cton_live <- function(pool) {
    cmass <- read_data(paste0("dave_cmass_", pool), layers = pft)
    nmass <- read_data(paste0("dave_nmass_", pool), layers = pft)

    cton_min <- read_data(paste0("dave_cton_", pool, "_min"), layers = pft)
    cton_max <- read_data(paste0("dave_cton_", pool, "_max"), layers = pft)
...
data <- read_data("dave_diameter")
ozflux_panel(src, "dave_diameter", site)
```

### Basal Area

```{r benchmark_basal_area}
quant <- defineQuantity("dave_patch_ba", "Basal Area", "m^2/ha")
ozflux_panel(src, quant, site)
```

### Soil Moisture

```{r benchmark_soil_moisture}
# The EucFACE reader is registered automatically from the bundled data.
...
data <- NULL
for (pool in pools) {
    var <- paste0("dave_cmass_standing_", pool)
...
data$date <- calc_date(data$Year, data$Day)
names(data) <- gsub("resp_", "", names(data))
...
layers <- paste0("soil_psi", 0:14)
data <- read_data("dave_soil_psi_layer", layers = layers)
data$date <- calc_date(data$Year, data$Day)
df_long <- pivot_longer(data, cols = layers, names_to = "layer",
                        values_to = "value")
df_long$layer_num <- as.integer(sub("^soil_psi", "", df_long$layer))
df_long$layer <- factor(df_long$layer,
                        levels = paste0("soil_psi", sort(unique(df_long$layer_num))))
labeller <- function(s) paste("layer", as.integer(sub("^soil_psi", "", s)))
ggplot(df_long) +
    geom_line(aes(x = date, y = value, color = layer)) +
    facet_wrap(~ layer,
               labeller = as_labeller(labeller)) +
...
psi_layers <- paste0("soil_psi", 0:14)
...
psi_long$layer_num <- as.integer(sub("^soil_psi", "", psi_long$layer))
psi_long$layer <- factor(
    psi_long$layer,
    levels = paste0("soil_psi", sort(unique(psi_long$layer_num)))
)
psi_labeller <- function(s) paste("layer", as.integer(sub("^soil_psi", "", s)))
...
psi_layers <- paste0("soil_psi", layers)

names(psi_data) <- gsub("soil_psi", "soil_psi", names(psi_data))
...
psi_long <- pivot_longer(psi_data, cols = all_of(paste0("soil_psi", layers)),
                         names_to = "layer", values_to = "psi") %>%
    mutate(layer = as.integer(sub("^soil_psi", "", layer)))
...
```{r g1_vs_swc}
g1 <- read_data("dave_g1", layers = pft)
g1 <- rename(g1, "g1" = pft)

data <- merge(g1, sw_data, by = c("Year", "Day"))
data$date <- calc_date(data$Year, data$Day)
data_long <- pivot_longer(data, cols = sw_layers, names_to = "layer",
                          values_to = "swc")
data_long$layer_num <- as.integer(sub("^sw_", "", data_long$layer))
data_long$layer <- factor(data_long$layer,
                          levels = paste0("sw_",
                                          sort(unique(data_long$layer_num))))

ggplot(data_long, aes(x = swc, y = g1, color = layer)) +
    geom_point(alpha = 0.4, size = 0.8) +
    xlab("Soil Moisture (0–1)") +
    ylab("g1 (kPa^0.5)") +
    labs(title = paste(site, "g1 vs Soil Moisture")) +
    theme(plot.title = element_text(hjust = 0.5))
```
...
ggplot(data_long, aes(x = swc, y = g1)) +
    geom_point(alpha = 0.4, size = 0.8, aes(color = layer)) +
    facet_wrap(~ layer, labeller = as_labeller(labeller)) +
    xlab("Soil Moisture (0–1)") +
    ylab("g1 (kPa^0.5)") +
    labs(title = paste(site, "g1 vs Soil Moisture")) +
    theme(plot.title = element_text(hjust = 0.5), legend.position = "none")
...
### g1 vs. Weighted Soil Water Potential

```{r g1_vs_weighted_psi}
weighted_psi <- read_data("dave_weighted_psi", layers = pft)
weighted_psi <- rename(weighted_psi, "weighted_psi" = pft)
data <- merge(g1, weighted_psi, by = c("Year", "Day"))
data$date <- calc_date(data$Year, data$Day)
ggplot(data, aes(x = weighted_psi, y = g1)) +
    geom_point(alpha = 0.4, size = 0.8) +
    xlab("Weighted Soil Water Potential (MPa)") +
    ylab("g1 (kPa^0.5)") +
    labs(title = paste(site, "g1 vs Weighted Soil Water Potential")) +
    theme(plot.title = element_text(hjust = 0.5))
```
...
names(data) <- gsub("cmass_", "", names(data))
...
# Rename e.g. MRS_cmass_leaf to leaf
names(data) <- gsub("nmass_", "", names(data))
...
names(data) <- gsub("cturnover_", "", names(data))
...
names(data) <- gsub("nturnover_", "", names(data))
...
vars <- c("dave_height", "dave_diameter", "dave_patch_ba", "dave_density")
...
mort_age <- read_data("dave_amort_age", layers = pft)
mort_greff <- read_data("dave_amort_greff", layers = pft)
...
```{r number_of_cohorts}
ozflux_plot_layerwise(src, "dave_ancohort", site, layers = pft)
```
...
```{r litter_carbon}
# surface: litter
# near-surface: grass and shrub live biomass
# 
litter_som_pools <- c("SURFSTRUCT", "SURFMETA")
som <- read_data("dave_sompool_cmass", layers = litter_som_pools)

standing <- read_data("dave_cmass_standing_leaf")
standing <- rename(standing, standing_leaf = "total")

data <- merge(som, standing, by = c("Year", "Day"))
data$leaf_litter <- rowSums(data[, ..litter_som_pools])
cols <- c("standing_leaf", "leaf_litter")
data$date <- calc_date(data$Year, data$Day)
data_long <- pivot_longer(data, cols = cols, names_to = "variable",
                          values_to = "value")
labeller <- function(v) gsub("_", " ", v)
ggplot(data_long) +
    geom_line(aes(x = date, y = value, color = variable)) +
    ylab("Carbon (kgC/m2)") +
    labs(title = paste(site, "Litter Carbon")) +
    theme(plot.title = element_text(hjust = 0.5))
```
...
The run description above should identify the model revision and simulation
settings. The parameters and R session below record how this report was rendered.
