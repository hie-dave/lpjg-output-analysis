---
title: "TellAus Benchmark Report for LPJ-GUESS Trunk (NARCliM2)"
author: "Drew Holzworth"
date: '`r strftime(Sys.time(), format = "%B %d, %Y")`'
output:
 html_document:
  toc: true
  css: tellaus.css
params:
  new_directory:
    value: "/home/drew/code/lpj-guess/.data/runs/narclim2/1deg/download/dave"
  new_name:
    value: NULL
  old_directory:
    value: "/home/drew/code/lpj-guess/.data/runs/narclim2/1deg/download/dave"
  old_name:
    value: NULL
  data_directory:
    value: "/home/drew/code/lpj-guess/.data"
---

```{r benchmark selection and script parameters, echo=FALSE}

### Select which benchmarks to do
do_lai <- TRUE
do_grass_dist <- FALSE # TODO: download dave_lai.out.gz from gadi when it's back online; also backup all outputs onto hie-storage
do_auseflux_gpp <- FALSE
do_auseflux_nee <- FALSE
do_auseflux_er <- FALSE
do_agb <- FALSE

debug <- FALSE

# Global script parameters.
settings <- list(
    data_path = params$data_directory,

    analysis_version = "tellme_dave  v0.1",

    plot_maps = TRUE,

    # Spatial extent for which data will be plotted. This can be:
    # - raster::extent
    # - two-column data.frame of gridcells
    # - NULL (to plot the entire spatial extent of the data)
    spatial_extent = NULL,

    # String used to describe the spatial subset.
    spatial_extent_id = "Australia",

    # Plot formatting arguments.
    map_overlay = "coastlines",

    # ME (Mean Error)
    # NME (Normalised Mean Error)
    # NMSE (Normalised Mean Square Error)
    # RMSE (Root Mean Squared Error)
    # NME_2 (NME with mean removed)
    # NMSE_2 (NMSE with mean removed)
    # NME_3 (NME with mean and variance removed)
    # NMSE_3 (NMSE with mean and variance removed)
    # r2_eff (Nash-Sutcliffe Model Efficiency)
    # r2 (Coefficient of Determination)
    # r (Pearson's PMCC)
    # m (gradient of linear fit mod = m * obs + c)
    # c (intercept of linear fit mod = m * obs + c)
    metrics = c("RMSE", "r2_eff", "r2", "NMSE"),

    text_multiplier = 2.5,

    annotation_text_size = 7,

    # Number of columns in which plots will be rendered.
    num_cols = 3,

    # Coordinates at which metrics/annotations will be rendered on maps.
    stats_lon = 125,
    stats_lat = -38,

    # The two time periods used for comparison of C3/C4 grass distribution.
    # The first time period is defined by the first two numbers.
    # The second time period is defined by the third and fourth numbers.
    grass_dist_years = c(1980, 1989, 2013, 2022)
)
```

```{r setup, include=FALSE}
# knitr options
knitr::opts_chunk$set(echo = FALSE,
                      message = FALSE,
                      out.width = "100%",
                      fig.height = 7,
                      warning = debug)

# kable options
opts <- options(knitr.kable.NA = "")

library("ggplot2")

# Set ggplot theme.
theme_set(theme_bw())

library("DGVMTools")
library("daveanalysis")
library("terra")
library("viridis")
library("mapproj")
library("pals")
library("knitr")
library("dplyr")
library("kableExtra")
library("tictoc")
library("lubridate")

# This helps RMarkdown locate fonts on some systems.
library("showtext")
showtext_auto()

# In debug mode, use log level 4 (all messages). Otherwise, use log level 1
# (warnings/errors only).
log_level <- ifelse(debug, 4, 1)
dave_config(log_level = log_level, log_file = "daveanalysis.log")

# Start timer.
tic()
```

# Description

These simulations are the Trunk version of the model run with NARCliM2.

# Settings {.tabset}

## Click here to hide Settings

## Click here to show settings

```{r settings}
# Simple setup
# dave_source <- defineSource(name = params$new_name,
#                             dir = params$new_directory,
#                             format = GUESS)
# trunk_source <- defineSource(name = params$old_name,
#                              dir = params$old_directory,
#                              format = GUESS)
# settings$simulations <- list(dave_source, trunk_source)

# NARCliM2 setup
# For now, use params$new_directory

# Files are:
# ${new_directory}/${GCM}/${RCM}/${EXPT}

sources <- list()
gcms <- list.dirs(params$new_directory, recursive = FALSE, full.names = FALSE)
for (gcm in gcms) {
    rcms <- list.dirs(file.path(params$new_directory, gcm), recursive = FALSE, full.names = FALSE)
    for (rcm in rcms) {
        # expts <- list.dirs(file.path(params$new_directory, gcm, rcm), recursive = FALSE, full.names = FALSE)
        expts <- "historical"
        for (expt in expts) {
            source_name <- paste(gcm, rcm, expt, sep = "_")
            source_dir <- file.path(params$new_directory, gcm, rcm, expt)
            if (!dir.exists(source_dir)) {
                stop("Directory not found: ", source_dir)
            }

            expt_short <- substr(expt, 1, 2)
            if (expt == "ssp126") {
                expt_short <- "S1"
            } else if (expt == "ssp370") {
                expt_short <- "S3"
            }
            gcm_short <- gsub("-", "", gcm)
            short_name <- paste(
                substr(gcm_short, 1, min(nchar(gcm_short), 3)),
                substr(rcm, max(1, nchar(rcm) - 1), nchar(rcm)),
                expt_short,
                sep = "_"
            )
            # Define source and add to list
            sources[[source_name]] <- defineSource(
                name = short_name,
                dir = source_dir,
                format = GUESS
            )
        }
    }
}


# Create a list of sources and store it in script settings.
settings$simulations <- sources

if (length(settings$simulations) == 0) {
    stop("No simulations were provided")
}

for (sim in settings$simulations) {
    cat(sim@name, ": ", sim@dir, "\n")
}
```

```{r silent preamble and preparation}
version_label <- paste(settings$analysis_version, settings$spatial_extent_id
                       , sep = "_")

# Prepare empty summary table of global numbers.
summary_col_names <- c("Quantity", "Unit")
for (simulation in settings$simulations) {
    summary_col_names <- append(summary_col_names, simulation@name)
}
summary_col_names <- append(summary_col_names
                            , c("Data", "Dataset", "Dataset ref."))
settings$summary_col_names <- summary_col_names

summary_table <- data.frame(check.names = FALSE, stringsAsFactors = FALSE)

# This table is completely empty; it will by built benchmark by benchmark.
metric_table <- data.frame(check.names = FALSE, stringsAsFactors = FALSE)

# Verify that observed data exists.
if (do_lai) {
    validate_lai(params$data_directory)
}

if (do_auseflux_gpp) {
    validate_auseflux(params$data_directory, "GPP")
}

if (do_auseflux_nee) {
    validate_auseflux(params$data_directory, "NEE")
}

if (do_auseflux_er) {
    validate_auseflux(params$data_directory, "ER")
}

if (do_agb) {
    validate_agb(params$data_directory)
}

# This list contains tables that gets passed in and out of each benchmark.
tables <- list(
    totals = summary_table,
    metrics = metric_table
)

# Fix overlapping x-axis labels by removing the overlapping labels.
fix_xaxis_label_overlap <- function(plt) {
    xlim <- layer_scales(plt)$x$get_limits()
    guide <- guide_axis(check.overlap = TRUE)
    return(plt + scale_x_continuous(guide = guide, limits = xlim))
}
print_plot <- function(plt) {
    if (!is.null(plt)) {
        plt <- fix_xaxis_label_overlap(plt)
        print(plt)
    }
}
```

***

# BoM LAI

Potential intro text about LAI data, links to online description of the data,
what to look for in the benchmark, etc.

```{r lai_benchmark}
if (do_lai) {
    lai <- benchmark_lai(settings, params, tables)
    tables <- lai$tables

    first_year <- lai$benchmark@first.year
    last_year <- lai$benchmark@last.year
    subtitle <- paste(first_year, last_year, sep = "-")
}
```

## Spatial patterns {.tabset}

### Absolute Values

```{r lai_plots_absolute_values}
if (do_lai && settings$plot_maps) {
    plt <- plotSpatial(lai$maps,
                       ncol = settings$num_cols,
                       legend.title = expression(m^{"2"}~m^{"-2"}),
                       map.overlay = settings$map_overlay,
                       text.multiplier = settings$text_multiplier,
                       title = "Absolute LAI values",
                       limits = c(0, 6),
                       subtitle = subtitle)
    print_plot(plt)
}
```

### Differences

```{r lai_plots_deltas}
if (do_lai && settings$plot_maps) {
    plot_absolute_deltas(lai, settings)
}
```

## Temporal trends {.tabset}

### Trend Value

```{r lai_plots_trend_values}
if (do_lai && settings$plot_maps) {
    plt <- plotSpatial(lai$trends
                       , layers = "Trend"
                       , ncol = settings$num_cols
                       , cols = rev(RColorBrewer::brewer.pal(11, "RdBu"))
                       , limits = c(-0.01, 0.01)
                       , drop.cuts = FALSE
                       , title = "Trend in LAI"
                       , legend.title =expression(m^{"2"}~m^{"-2"}~month^{"-2"})
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , subtitle = subtitle)
    print_plot(plt)
}
```

### Trend Significance

```{r lai_plots_trend_significance}
if (do_lai && settings$plot_maps) {
    plt <- plotSpatial(lai$trends
                       , layers = "p.value"
                       , ncol = settings$num_cols
                       , cols = viridis::turbo(6)
                       , cuts = c(0, 0.001, 0.01, 0.05, 0.1, 0.5, 1.0)
                       , drop.cuts = FALSE
                       , title = "Significance in LAI Trends"
                       , legend.title = "p value"
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , subtitle = subtitle)
    print_plot(plt)
}
```

### Trend Differences

```{r lai_plots_trend_deltas}
if (do_lai && settings$plot_maps) {
    plt <- plotSpatialComparison(lai$comparisons[["Trend"]]
                                 , ncol = settings$num_cols
                                 , drop.cuts = NULL
                                 , title = "Difference Trend in LAI"
                                 , legend.title = expression(m^{"2"}~m^{"-2"}~month^{"-2"})
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , subtitle = subtitle)
    print_plot(plt)
}
```

### Significant Trends

```{r lai_plots_trends}
if (do_lai && settings$plot_maps) {
    plt <- plotSpatial(lai$trends
                       , layers = "Significant_Trend"
                       , ncol = settings$num_cols
                       , cols = rev(RColorBrewer::brewer.pal(11, "RdBu"))
                       , limits = c(-0.01, 0.01)
                       , drop.cuts = FALSE
                       , title = "Significant Trend in LAI (p-value < 0.05)"
                       , legend.title = expression(m^{"2"}~m^{"-2"}~month^{"-2"})
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , subtitle = subtitle)
    print_plot(plt)
}
```

## Seasonal Analysis {.tabset}

Definition of seasonal concentration and phase follow Kelley *et al.* 2013. **Seasonal concentration** equals one if the variable is concentrated all in one month, and is zero if it is spread evenly across all months.  The **seasonal phase** is a measure of the peak of the season - it is not strictly the maximum monthly, but rather the average of all months when considering months as an angle in the complex plane.

### Seasonal Phase

```{r lai_plots_phases}
if (do_lai && settings$plot_maps) {
    plt <- plotSpatialComparison(lai$comparisons[["Seasonal"]]
                                 , do.phase = TRUE
                                 , type = "values"
                                 , cuts = 0:12
                                 , override.cols = pals::ocean.phase(12)
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , title = "Seasonal Phase LAI"
                                 , subtitle = subtitle)
    print_plot(plt)
}
```

### Phase Differences

```{r lai_plots_phase_deltas}
if (do_lai && settings$plot_maps) {
    plt <- plotSpatialComparison(lai$comparisons[["Seasonal"]]
                                 , do.phase = TRUE
                                 , cuts = seq(-6, 6)
                                 , symmetric.scale = FALSE
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , subtitle = subtitle)
    print_plot(plt)
}
```

### Seasonal Concentration

```{r lai_plots_seasonsal_concentration}
if (do_lai && settings$plot_maps) {
    plt <- plotSpatialComparison(lai$comparisons[["Seasonal"]]
                                 , type = "values"
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , subtitle = subtitle)
    print_plot(plt)
}
```

### Concentration Differences

```{r lai_plots_concentration_deltas}
if (do_lai && settings$plot_maps) {
    plt <- plotSpatialComparison(lai$comparisons[["Seasonal"]]
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , subtitle = subtitle)
    print_plot(plt)
}
```

## {.unlisted .unnumbered}

<a href="#top">Back to top</a>

# C3/C4 Distribution

```{r grass_dist}
if (do_grass_dist) {
    grass_benchmark <- benchmark_grasses(settings, params, tables)
    tables <- grass_benchmark$tables
    grass_first_year <- min(grass_benchmark$first_year)
    grass_last_year <- max(grass_benchmark$last_year)
    grass_subtitle <- paste(grass_first_year, grass_last_year, sep = "-")
    grass_delta_subtitle <- paste0(params$new_name, " - ", params$old_name
                                   , " (", grass_subtitle, ")")
}
```

## Spatial Patterns (Fixed Time Period) {.tabset}

### Absolute Values

```{r grass_dist_plots_abs}
if (do_grass_dist && settings$plot_maps) {
    log_diag("first.year=", grass_first_year, ", last.year=", grass_last_year)
    plt <- plotSpatial(grass_benchmark$period_maps
                       , legend.title = expression(1)
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , title = "C4 Fraction")
                       #, subtitle = subtitle)
    print_plot(plt)
}
```

### Differences

```{r grass_dist_plots_deltas}
if (do_grass_dist && settings$plot_maps) {
    years <- settings$grass_dist_years
    r0 <- paste0(years[1], "-", years[2])
    r1 <- paste0(years[3], "-", years[4])
    title <- paste0("C4 Fraction Change from (", r0, ") - (", r1, ")")
    plt <- plotSpatial(grass_benchmark$diffs
                       , legend.title = expression((0-1))
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , title = title
                       , subtitle = character(0)
                       , panel.bg.col = "white"
                       , cols = rev(RColorBrewer::brewer.pal(11, "RdBu"))
                       , limits = c(-1, 1))
    print_plot(plt)
}
```

## Spatial Patterns (Full Time Period) {.tabset}

### Absolute Values

```{r grass_dist_plots_abs_values}
if (do_grass_dist && settings$plot_maps) {
    plt <- plotSpatial(grass_benchmark$means
                       , ncol = settings$num_cols
                       , legend.title = expression((0-1))
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , title = "C4 Fraction"
                       , subtitle = grass_subtitle)
    print_plot(plt)
}
```

### Differences

```{r grass_dist_plots_abs_differences}
if (do_grass_dist && settings$plot_maps && !is.null(grass_benchmark$comparisons)) {
    plot_absolute_deltas(grass_benchmark, settings)
    # plt <- plotSpatialComparison(grass_benchmark$comparisons[["Values"]]
    #                              , ncol = settings$num_cols
    #                              , legend.title = expression(Delta)
    #                              , text.multiplier = settings$text_multiplier
    #                              , map.overlay = settings$map_overlay
    #                              , title = "C4 Fraction Biases"
    #                              , subtitle = grass_delta_subtitle
    #                              , panel.bg.col = "white")
    # print_plot(plt)
}
```

## Temporal Trends {.tabset}

### Trend Value

```{r grass_dist_plots_trend_values}
if (do_grass_dist && settings$plot_maps && !is.null(grass_benchmark$trends) && length(grass_benchmark$trends) > 0) {
    plt <- plotSpatial(grass_benchmark$trends
                       , layers = "Trend"
                       , ncol = settings$num_cols
                       , cols = rev(RColorBrewer::brewer.pal(11, "RdBu"))
                       , limits = c(-6e-5, 6e-5)
                       , drop.cuts = FALSE
                       , title = "Trend in C4 Fraction"
                       , legend.title = expression(~month^{"-2"})
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier)
    print_plot(plt)
}
```

### Trend Significance

```{r grass_dist_plots_trend_significances}
if (do_grass_dist && settings$plot_maps && !is.null(grass_benchmark$trends) && length(grass_benchmark$trends) > 0) {
    plt <- plotSpatial(grass_benchmark$trends
                       , layers = "p.value"
                       , ncol = settings$num_cols
                       , cols = viridis::turbo(6)
                       , cuts = c(0, 0.001, 0.01, 0.05, 0.1, 0.5, 1.0)
                       , drop.cuts = FALSE
                       , title = "Significance in C4 Fraction Trends"
                       , legend.title = "p value"
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , subtitle = grass_subtitle)
    print_plot(plt)
}
```

### Trend Differences

```{r grass_dist_plots_trend_deltas}
if (do_grass_dist && settings$plot_maps && !is.null(grass_benchmark$comparisons)) {
    plt <- plotSpatialComparison(grass_benchmark$comparisons[["Trend"]]
                                 , ncol = settings$num_cols
                                 , drop.cuts = NULL
                                 , title = "Difference Trend in C4 Fraction"
                                 , legend.title = expression((0-1))
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , subtitle = grass_delta_subtitle)
    print_plot(plt)
}
```

### Significant Trends

```{r grass_dist_plots_significant_trends}
if (do_grass_dist && settings$plot_maps && !is.null(grass_benchmark$trends) && length(grass_benchmark$trends) > 0) {
    plt <- plotSpatial(grass_benchmark$trends
                       , layers = "Significant_Trend"
                       , ncol = settings$num_cols
                       , cols = rev(RColorBrewer::brewer.pal(11, "RdBu"))
                       , limits = c(-0.01, 0.01)
                       , drop.cuts = FALSE
                       , title = "Significant Trend in LAI (p-value < 0.05)"
                       , legend.title = expression(m^{"2"}~m^{"-2"}~month^{"-2"})
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , subtitle = grass_subtitle)
    print_plot(plt)
}
# todo: add seasonal analysis once we have observations of c3/c4 distribution
```

## {.unlisted .unnumbered}

<a href="#top">Back to top</a>

# AusEFlux GPP

Intro, data links, etc.

```{r auseflux_gpp}
if (do_auseflux_gpp) {
    # auseflux_gpp <- benchmark_auseflux("GPP", settings, params, tables)
    auseflux_gpp <- benchmark_gpp(params, settings, tables)
    tables <- auseflux_gpp$tables
    benchmark <- auseflux_gpp$benchmark
    auseflux_subtitle <- paste0(benchmark@first.year, "-", benchmark@last.year)
}
```

## Spatial Patterns {.tabset}

### Absolute Values

```{r auseflux_gpp_plots_absolute}
if (do_auseflux_gpp && settings$plot_maps) {
    plt <- plotSpatial(auseflux_gpp$maps
                       , ncol = settings$num_cols
                       , legend.title = expression(gC~m^{"-2"}~year^{"-1"})
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , title = "Absolute GPP values"
                       , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Differences

```{r auseflux_gpp_plots_deltas}
if (do_auseflux_gpp && settings$plot_maps) {
    plot_absolute_deltas(auseflux_gpp, settings)
    # plt <- plotSpatialComparison(auseflux_gpp$comparisons[["Values"]]
    #                              , ncol = settings$num_cols
    #                              , legend.title = expression(Delta~gC~m^{"-2"}~year^{"-1"})
    #                              , map.overlay = settings$map_overlay
    #                              , text.multiplier = settings$text_multiplier
    #                              , title = "GPP biases"
    #                              , subtitle = auseflux_subtitle
    #                              , panel.bg.col = "white")
    # print_plot(plt)
}
```

## Temporal Trends {.tabset}

### Trend Value

```{r auseflux_gpp_plots_trend_values}
if (do_auseflux_gpp && settings$plot_maps) {
    plt <- plotSpatial(auseflux_gpp$trends
                       , layers = "Trend"
                       , ncol = settings$num_cols
                       , cols = rev(RColorBrewer::brewer.pal(11, "RdBu"))
                       , limits = c(-0.01, 0.01)
                       , drop.cuts = FALSE
                       , title = "Trend in GPP"
                       , legend.title = expression(gC~m^{"-2"}~month^{"-2"})
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Trend Significance

```{r auseflux_gpp_plots_trend_significance}
if (do_auseflux_gpp && settings$plot_maps) {
    plt <- plotSpatial(auseflux_gpp$trends
                       , layers = "p.value"
                       , ncol = settings$num_cols
                       , cols = viridis::turbo(6)
                       , cuts = c(0, 0.001, 0.01, 0.05, 0.1, 0.5, 1.0)
                       , drop.cuts = FALSE
                       , title = "Significance in GPP Trends"
                       , legend.title = "p value"
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Trend Differences

```{r auseflux_gpp_plots_trend_deltas}
if (do_auseflux_gpp && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_gpp$comparisons[["Trend"]]
                                 , ncol = settings$num_cols
                                 , drop.cuts = NULL
                                 , title = "Difference Trend in GPP"
                                 , legend.title = expression(Delta~gC~m^{"-2"}~month^{"-2"})
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Significant Trends

```{r auseflux_gpp_plots_significant_trends}
if (do_auseflux_gpp && settings$plot_maps) {
    plt <- plotSpatial(auseflux_gpp$trends
                       , layers = "Significant_Trend"
                       , ncol = settings$num_cols
                       , cols = rev(RColorBrewer::brewer.pal(11, "RdBu"))
                       , limits = c(-0.01, 0.01)
                       , drop.cuts = FALSE
                       , title = "Significant Trend in GPP (p-value < 0.05)"
                       , legend.title = expression(gC~m^{"-2"}~month^{"-2"})
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

## Seasonal Analysis {.tabset}

Definition of seasonal concentration and phase follow Kelley *et al.* 2013.  **Seasonal concentration** equals one if the variable is concentrated all in one month, and is zero if it is spread even across all months.  The **seasonal phase** is a measure of the peak of the season - it is not strictly the maximum monthly, but rather the average of all months when considering months as an angle in the complex plane.  

### Seasonal Phase

```{r auseflux_gpp_plots_phases}
if (do_auseflux_gpp && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_gpp$comparisons[["Seasonal"]]
                                 , do.phase = TRUE
                                 , type = "values"
                                 , cuts = 0:12
                                 , override.cols = pals::ocean.phase(12)
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , title = "Seasonal Phase GPP"
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Phase Differences

```{r auseflux_gpp_plots_phase_deltas}
if (do_auseflux_gpp && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_gpp$comparisons[["Seasonal"]]
                                 , do.phase = TRUE
                                 , cuts = seq(-6, 6)
                                 , symmetric.scale = FALSE
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Seasonal Concentration

```{r auseflux_gpp_plots_seasonal_concentration}
if (do_auseflux_gpp && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_gpp$comparisons[["Seasonal"]]
                                 , type = "values"
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Concentration Differences

```{r auseflux_gpp_plots_concentration_deltas}
if (do_auseflux_gpp && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_gpp$comparisons[["Seasonal"]]
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

## {.unlisted .unnumbered}

<a href="#top">Back to top</a>

# AusEFlux NEE

Intro, data links, etc.

```{r auseflux_nee}
if (do_auseflux_nee) {
    auseflux_nee <- benchmark_auseflux("NEE", settings, params, tables)
    tables <- auseflux_nee$tables
    benchmark <- auseflux_nee$benchmark
    auseflux_subtitle <- paste0(benchmark@first.year, "-", benchmark@last.year)
}
```

## Spatial Patterns {.tabset}

### Absolute Values

```{r auseflux_nee_plots_absolute}
if (do_auseflux_nee && settings$plot_maps) {
    plt <- plotSpatial(auseflux_nee$maps
                       , ncol = settings$num_cols
                       , legend.title = expression(gC~m^{"-2"}~year^{"-1"})
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , title = "Absolute NEE Values"
                       , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Differences

```{r auseflux_nee_plots_deltas}
if (do_auseflux_nee && settings$plot_maps) {
    plot_absolute_deltas(auseflux_nee, settings)
    # plt <- plotSpatialComparison(auseflux_nee$comparisons[["Values"]]
    #                              , ncol = settings$num_cols
    #                              , legend.title = expression(Delta~gC~m^{"-2"}~year^{"-1"})
    #                              , map.overlay = settings$map_overlay
    #                              , text.multiplier = settings$text_multiplier
    #                              , title = "NEE biases"
    #                              , subtitle = auseflux_subtitle
    #                              , panel.bg.col = "white")
    # print_plot(plt)
}
```

## Temporal Trends {.tabset}

### Trend Value

```{r auseflux_nee_plots_trend_values}
if (do_auseflux_nee && settings$plot_maps) {
    plt <- plotSpatial(auseflux_nee$trends
                       , layers = "Trend"
                       , ncol = settings$num_cols
                       , cols = rev(RColorBrewer::brewer.pal(11, "RdBu"))
                       , limits = c(-0.01, 0.01)
                       , drop.cuts = FALSE
                       , title = "Trend in NEE"
                       , legend.title = expression(gC~m^{"-2"}~month^{"-2"})
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Trend Significance

```{r auseflux_nee_plots_trend_significance}
# TODO: create a variable for cuts, and change the 6 in the number of colours
# to be the length of this variable.
if (do_auseflux_nee && settings$plot_maps) {
    plt <- plotSpatial(auseflux_nee$trends
                       , layers = "p.value"
                       , ncol = settings$num_cols
                       , cols = viridis::turbo(6)
                       , cuts = c(0, 0.001, 0.01, 0.05, 0.1, 0.5, 1.0)
                       , drop.cuts = FALSE
                       , title = "Signifiance in NEE Trends"
                       , legend.title = "p value"
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Trend Differences

```{r auseflux_nee_plots_trend_deltas}
if (do_auseflux_nee && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_nee$comparisons[["Trend"]]
                                 , ncol = settings$num_cols
                                 , drop.cuts = NULL
                                 , title = "Difference Trend in NEE"
                                 , legend.title = expression(Delta~gC~m^{"-2"}~month^{"-2"})
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Significant Trends

```{r auseflux_nee_plots_significant_trends}
if (do_auseflux_nee && settings$plot_maps) {
    plt <- plotSpatial(auseflux_nee$trends
                       , layers = "Significant_Trend"
                       , ncol = settings$num_cols
                       , cols = rev(RColorBrewer::brewer.pal(11, "RdBu"))
                       , limits = c(-0.01, 0.01)
                       , drop.cuts = FALSE
                       , title = "Significant Trend in NEE (p-value < 0.05)"
                       , legend.title = expression(gC~m^{"-2"}~month^{"-2"})
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

## Seasonal Analysis {.tabset}

Definition of seasonal concentration and phase follow Kelley *et al.* 2013.  **Seasonal concentration** equals one if the variable is concentrated all in one month, and is zero if it is spread even across all months.  The **seasonal phase** is a measure of the peak of the season - it is not strictly the maximum monthly, but rather the average of all months when considering months as an angle in the complex plane.  

### Seasonal Phase

```{r auseflux_nee_plots_phases}
if (do_auseflux_nee && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_nee$comparisons[["Seasonal"]]
                                 , do.phase = TRUE
                                 , type = "values"
                                 , cuts = 0:12
                                 , override.cols = pals::ocean.phase(12)
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , title = "Seasonal Phase NEE"
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Phase Differences

```{r auseflux_nee_plots_phase_deltas}
if (do_auseflux_nee && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_nee$comparisons[["Seasonal"]]
                                 , do.phase = TRUE
                                 , cuts = seq(-6, 6)
                                 , symmetric.scale = FALSE
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Seasonal Concentration

```{r auseflux_nee_plots_seasonal_concentration}
if (do_auseflux_nee && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_nee$comparisons[["Seasonal"]]
                                 , type = "values"
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Concentration Differences

```{r auseflux_nee_plots_concentration_deltas}
if (do_auseflux_nee && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_nee$comparisons[["Seasonal"]]
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

## {.unlisted .unnumbered}

<a href="#top">Back to top</a>

# AusEFlux ER

Intro, data links, etc.

```{r auseflux_er}
if (do_auseflux_er) {
    auseflux_er <- benchmark_auseflux("ER", settings, params, tables)
    tables <- auseflux_er$tables
    benchmark <- auseflux_er$benchmark
    auseflux_subtitle <- paste0(benchmark@first.year, "-", benchmark@last.year)
}
```

## Spatial Patterns {.tabset}

### Absolute Values

```{r auseflux_er_plots_absolute}
if (do_auseflux_er && settings$plot_maps) {
    plt <- plotSpatial(auseflux_er$maps
                       , ncol = settings$num_cols
                       , legend.title = expression(gC~m^{"-2"}~year^{"-1"})
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , title = "Absolute ER values"
                       , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Differences

```{r auseflux_er_plots_deltas}
if (do_auseflux_er && settings$plot_maps) {
    plot_absolute_deltas(auseflux_er, settings)
    # plt <- plotSpatialComparison(auseflux_er$comparisons[["Values"]]
    #                              , ncol = settings$num_cols
    #                              , legend.title = expression(Delta~gC~m^{"-2"}~year^{"-1"})
    #                              , map.overlay = settings$map_overlay
    #                              , text.multiplier = settings$text_multiplier
    #                              , title = "ER biases"
    #                              , subtitle = auseflux_subtitle
    #                              , panel.bg.col = "white")
    # print_plot(plt)
}
```

## Temporal Trends {.tabset}

### Trend Value

```{r auseflux_er_plots_trend_values}
if (do_auseflux_er && settings$plot_maps) {
    plt <- plotSpatial(auseflux_er$trends
                       , layers = "Trend"
                       , ncol = settings$num_cols
                       , cols = rev(RColorBrewer::brewer.pal(11, "RdBu"))
                       , limits = c(-0.01, 0.01)
                       , drop.cuts = FALSE
                       , title = "Trend in ER"
                       , legend.title = expression(Delta~gC~m^{"-2"}~month^{"-2"})
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Trend Significance

```{r auseflux_er_plots_trend_significance}
if (do_auseflux_er && settings$plot_maps) {
    plt <- plotSpatial(auseflux_er$trends
                       , layers = "p.value"
                       , ncol = settings$num_cols
                       , cols = viridis::turbo(6)
                       , cuts = c(0, 0.001, 0.01, 0.05, 0.1, 0.5, 1.0)
                       , drop.cuts = FALSE
                       , title = "Significance in ER Trends"
                       , legend.title = "p value"
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Trend Differences

```{r auseflux_er_plots_trend_deltas}
if (do_auseflux_er && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_er$comparisons[["Trend"]]
                                 , ncol = settings$num_cols
                                 , drop.cuts = NULL
                                 , title = "Difference Trend in ER"
                                 , legend.title = expression(Delta~gC~m^{"-2"}~month^{"-2"})
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Significant Trends

```{r auseflux_er_plots_significant_trends}
if (do_auseflux_er && settings$plot_maps) {
    plt <- plotSpatial(auseflux_er$trends
                       , layers = "Significant_Trend"
                       , ncol = settings$num_cols
                       , cols = rev(RColorBrewer::brewer.pal(11, "RdBu"))
                       , limits = c(-0.01, 0.01)
                       , drop.cuts = FALSE
                       , title = "Significant Trend in ER (p-value < 0.05)"
                       , legend.title = expression(gC~m^{"-2"}~month^{"-2"})
                       , map.overlay = settings$map_overlay
                       , text.multiplier = settings$text_multiplier
                       , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

## Seasonal Analysis {.tabset}

Definition of seasonal concentration and phase follow Kelley *et al.* 2013.  **Seasonal concentration** equals one if the variable is concentrated all in one month, and is zero if it is spread even across all months.  The **seasonal phase** is a measure of the peak of the season - it is not strictly the maximum monthly, but rather the average of all months when considering months as an angle in the complex plane.  

### Seasonal Phase

```{r auseflux_er_plots_phases}
if (do_auseflux_er && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_er$comparisons[["Seasonal"]]
                                 , do.phase = TRUE
                                 , type = "values"
                                 , cuts = 0:12
                                 , override.cols = pals::ocean.phase(12)
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , title = "Seasonal Phase ER"
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Phase Differences

```{r auseflux_er_plots_phase_deltas}
if (do_auseflux_er && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_er$comparisons[["Seasonal"]]
                                 , do.phase = TRUE
                                 , cuts = seq(-6, 6)
                                 , symmetric.scale = FALSE
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , title = "Difference Seasonal Phase ER"
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Seasonal Concentration

```{r auseflux_er_plots_seasonal_concentration}
if (do_auseflux_er && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_er$comparisons[["Seasonal"]]
                                 , type = "values"
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , title = "Seasonal Concentration ER"
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

### Concentration Differences

```{r auseflux_er_plots_concentration_deltas}
if (do_auseflux_er && settings$plot_maps) {
    plt <- plotSpatialComparison(auseflux_er$comparisons[["Seasonal"]]
                                 , ncol = settings$num_cols
                                 , map.overlay = settings$map_overlay
                                 , text.multiplier = settings$text_multiplier
                                 , title = "Difference Seasonal Concentration ER"
                                 , subtitle = auseflux_subtitle)
    print_plot(plt)
}
```

## {.unlisted .unnumbered}

<a href="#top">Back to top</a>

# Above-Ground Biomass

Potential intro text about AGB data, links to online description of the data,
what to look for in the benchmark, etc.

```{r benchmark_agb}
if (do_agb) {
    agb <- benchmark_agb(params, settings, tables)
    tables <- agb$tables

    first_year <- agb$benchmark@first.year
    last_year <- agb$benchmark@last.year
    subtitle <- paste(first_year, last_year, sep = "-")
}
```

## Spatial patterns {.tabset}

### Absolute Values

```{r agb_plots_absolute_values}
if (do_agb && settings$plot_maps) {
    plt <- plotSpatial(agb$maps,
                       ncol = settings$num_cols,
                       legend.title = expression(kg~m^{"-2"}),
                       map.overlay = settings$map_overlay,
                       text.multiplier = settings$text_multiplier,
                       title = "Absolute AGB values",
                       limits = c(0, 60),
                       subtitle = subtitle)
    print_plot(plt)
}
```

### Differences

```{r agb_plots_deltas}
if (do_agb && settings$plot_maps) {
    plot_absolute_deltas(agb, settings)
}
```

## {.unlisted .unnumbered}

<a href="#top">Back to top</a>

***

# Global Summary Tables

## Data Comparisons

TODO

```{r global_summary}
summary_table <- tables[["totals"]]
```

## Model-Data Agreement Metrics

```{r global_metrics}
metric_table <- tables[["metrics"]]
if (nrow(metric_table) > 0) {
    if ("Metric" %in% names(metric_table)) {
        metric_table$Metric <- gsub("r2_eff", "NSE", metric_table$Metric)
        metric_table$Metric <- gsub("r2", "R^2^", metric_table$Metric)
    }

    # TODO: make use of these columns?
    to_remove <- c("Data bootstrap", "Data mean", "Dataset ref.")
    metric_table <- metric_table[, setdiff(names(metric_table), to_remove)]

    kable(metric_table) %>% kable_styling(full_width = TRUE)
}

# TODO: save the table in each model run directory?
# filename <- paste0("summary_metric_", version_label, ".RData")
# for (simulation in settings$simulations) {
#     saveRDS(metric_table, file.path(simulation@dir, filename))
# }
```

# Timing
Total run time:
``` {r time}
# report total run time
toc()
```
