...
data <- read_data(src, "dave_acpool", site, layers = cpool_layers)@data
...
data <- read_data(src, "dave_clitter", site, layers = layers)@data
data$date <- calc_date(data$Year, data$Day)
...
cmass_litter_leaf <- read_data(src, "dave_cmass_standing_dead_leaf", site,
                               layers = pft)@data
cmass_litter_root <- read_data(src, "dave_cmass_standing_dead_root", site,
                               layers = pft)@data
cmass_litter_sap <- read_data(src, "dave_cmass_standing_dead_sap", site,
                              layers = pft)@data
cmass_litter_heart <- read_data(src, "dave_cmass_standing_dead_heart", site,
                                layers = pft)@data
cmass_litter_repr <- read_data(src, "dave_cmass_standing_dead_repr", site,
                               layers = pft)@data

# Drop Lon/Lat cols.
cmass_litter_leaf <- select(cmass_litter_leaf, -Lon, -Lat)
cmass_litter_root <- select(cmass_litter_root, -Lon, -Lat)
cmass_litter_sap <- select(cmass_litter_sap, -Lon, -Lat)
cmass_litter_heart <- select(cmass_litter_heart, -Lon, -Lat)
cmass_litter_repr <- select(cmass_litter_repr, -Lon, -Lat)

# Rename pft column to litter_X
names(cmass_litter_leaf) <- gsub(pft, "leaf", names(cmass_litter_leaf))
names(cmass_litter_root) <- gsub(pft, "root", names(cmass_litter_root))
names(cmass_litter_sap) <- gsub(pft, "sap", names(cmass_litter_sap))
names(cmass_litter_heart) <- gsub(pft, "heart", names(cmass_litter_heart))
names(cmass_litter_repr) <- gsub(pft, "repr", names(cmass_litter_repr))

data <- merge(cmass_litter_leaf, cmass_litter_root, by = c("Year", "Day"))
data <- merge(data, cmass_litter_sap, by = c("Year", "Day"))
data <- merge(data, cmass_litter_heart, by = c("Year", "Day"))
data <- merge(data, cmass_litter_repr, by = c("Year", "Day"))
data$date <- calc_date(data$Year, data$Day)
...
```{r clitter}
layers <- c("SURFMETA", "SURFSTRUCT", "SURFCWD", "SURFFWD")
data <- read_data(src, "dave_nlitter", site, layers = layers)@data
data$date <- calc_date(data$Year, data$Day)
data_long <- pivot_longer(data, cols = layers, names_to = "layer",
                          values_to = "value") %>%
    dplyr::mutate(layer = factor(layer, levels = layers))
...
data <- read_data(src, "dave_soil_psi_layer", site, layers = layers)@data
data$date <- calc_date(data$Year, data$Day)
