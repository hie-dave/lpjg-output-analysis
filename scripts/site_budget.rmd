...
eucface_file <- file.path(eucface_path, "EucFACE_soil_moisture_1m_sitecol.csv")
...
nee_data <- rename(nee_data, nee = all_of(src@name), observed_nee = observed)
# We don't have NPP observations, so npp_data comes out with the right colname.
gpp_data <- rename(gpp_data, gpp = all_of(src@name), observed_gpp = observed)
resp_data <- rename(resp_data, resp = all_of(src@name),
                    observed_resp = observed)
...
data <- read_data("dave_soil_nflux", layers = nflux_cols)
data$date <- calc_date(data$Year, data$Day)
...
data <- rename(data, fixation = "dnfix",
               gross_mineralisation = "nmin_gross",
               immobilisation = "nimmob",
               net_mineralisation = "nmin_net",
               org_leach = "norg_leach",
               min_leach = "nmin_leach",
               volatilisation = "nvolatilisation",
               gas_emissions = "nloss_gas",
               avail = "avail")
...
df_long$layer <- factor(df_long$layer,
                        levels = paste0("soil_psi",
                                        sort(unique(df_long$layer_num))))
...
names(psi_data) <- gsub("soil_psi", "psi_", names(psi_data))
...
data <- merge(data, psi_data[, c("Year", "Day", "psi_mean")], by = c("Year", "Day"))
ggplot(data, aes(x = psi_mean, y = wscal)) +
    geom_point(color = "#0072B2") +
    xlab("Weighted Soil Water Potential (MPa)") +
    ylab("Water Stress (0-1)") +
    labs(title = paste(site, "Water stress vs unweighted mean soil water potential")) +
    theme(plot.title = element_text(hjust = 0.5))
```
...
nmass_storage <- remove_cols(nmass_storage, "Day")
